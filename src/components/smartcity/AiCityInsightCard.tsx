...
import { AlertTriangle, Cpu, CheckCircle2, ArrowRight, Users, ShieldAlert } from 'lucide-react';
import { useCivic } from '../../context/CivicContext';

export const AiCityInsightCard: React.FC = () => {
  const { aiCityInsight, setActiveView } = useCivic();
  ...
  return (
    <div className="gov-card rounded-lg p-4 bg-white border border-slate-200 shadow-sm space-y-3">
      ...
      <div className="p-2.5 rounded bg-blue-50 border border-blue-200 space-y-1 text-xs text-blue-950">
      ...
      <div className="pt-1 flex items-center justify-between text-xs text-slate-600 border-t border-slate-100">
        <span className="flex items-center space-x-1">
          ...
          <span>Predicted Impact: <strong className="text-slate-900 font-mono">~{aiCityInsight.predictedImpactCitizens.toLocaleString()} citizens</strong></span>
        </span>
        <button
          onClick={() => setActiveView('disaster_alerts')}
          className="text-blue-800 font-semibold text-[11px] hover:underline"
        >
          Dispatch Geo-Alert →
        </button>
      </div>
